...
window.initMap = function() {
    ...
    // We don't need to do anything here anymore, as `whenDefined` is more reliable.
    console.log("Google Maps JS API script has loaded.");
};
...
document.addEventListener('alpine:init', () => {
    ...
    // --- v36.0 DEFINITIVE GOOGLE ADDRESS COMPONENT WITH `whenDefined` ---
    Alpine.data('googleAddressInput', (dispatch, fieldName) => ({
        ...
        initializeAutocomplete() {
            ...
            if (autocompleteElement) {
                ...
                console.error("Autocomplete element not found in refs.");
            }
        },
        init() {
            ...
            // This is the robust fix. It waits until the browser has fully
            // defined the <gmp-place-autocomplete> element before we try to use it.
            customElements.whenDefined('gmp-place-autocomplete').then(() => {
                this.initializeAutocomplete();
            }).catch(error => {
                console.error("Error waiting for gmp-place-autocomplete component:", error);
            });
        }
    }));
